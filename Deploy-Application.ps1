...
Try {
	...
	[string]$appVendor = 'Jamie Gates'
	[string]$appName = 'Platinum'
	[string]$appVersion = '3.0.1'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.2'
	[string]$appScriptDate = '2022.08.02'
	...
	$CloseApps = "msedge=Microsoft Edge,chrome=Google Chrome,firefox=Firefox,MSACCESS=Access"
	...
	$PrereqApp1InstallPath = "$PSScriptRoot\Files\msodbcsql.msi"
	...
	$RemoveAppNamesMSI = @("Microsoft ODBC Driver 17 for SQL Server", "Microsoft ODBC Driver 11 for SQL Server", "Microsoft ODBC Driver 13 for SQL Server", "Microsoft ODBC Driver 13.1 for SQL Server", "Microsoft ODBC Driver 18 for SQL Server")
	#$RemoveAppNamesEXE = @("")

	##* Application uninstall parameters.
	# 64-bit application
	# 64-bit application uninstall name.
	$64bitAppUninstallName = "Platinum"
	...
	[string[]]$appShortcutDirs = @("${Env:ProgramFiles}\Tuolumne County\Platinum", "${Env:ProgramData}\Microsoft\Windows\Start Menu\Programs", "${Env:PUBLIC}\Desktop")
	...
	$32bitAppInstallPath = "${Env:ProgramFiles}\Tuolumne County\Platinum"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Foreach ($appShortcutDir in $appShortcutDirs) {
			Try {
				...
				New-Shortcut -Path "$appShortcutDir\$64bitAppInstallName.lnk" -TargetPath "CONFIDENTIAL" -IconLocation "$envProgramFiles\Tuolumne County\Platinum\Assets\Diamond.ico" -WindowStyle "Maximized" -Description "Platinum"
			}
	...
	ElseIf ($deploymentType -ieq 'Uninstall') {
		...
		# Show install completed prompt.
		Write-Log -Message "Showing installation completed prompt."
		Show-InstallationPrompt -Title "Install Completed"  -Icon "Information" -PersistPrompt -ButtonMiddleText "OK" -Message "Your uninstallation of $apName ($apversion) has completed.`n `n Please close any remaining prompts/windows that may have opened.`n `n Questions or issues? `n Please contact the IT Service Desk: `n ITServiceDesk@co.tuolumne.ca.us"	
